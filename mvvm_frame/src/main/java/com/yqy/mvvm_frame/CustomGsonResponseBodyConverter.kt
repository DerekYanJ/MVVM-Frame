        ...
        if (apiResponse.code == 0) {
            try {
                try {
                    ...
                    e1.printStackTrace()
                    throw YError(-2, "JSON解析失败${e1.message}")
                }
            } catch (e: Exception) {
                e.printStackTrace()
                if (e is YError) {
                    ...
                    throw YError(-2, "JSON解析失败${e.message}")
                }
            } finally {
                value.close()
            }
            ...
            if (apiResponse.code == -1) {
                throw YError(-1, "服务器出现异常！")
            }
            throw YError(apiResponse.code, apiResponse.msg)
        }
    }
}
